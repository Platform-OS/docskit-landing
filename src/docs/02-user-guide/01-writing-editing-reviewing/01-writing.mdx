...
<Message>**Frontmatter** is metadata that you have to include at the beginning of an MDX file when working with DocsKit. It is enclosed by triple dashes `---` and is used to provide additional information or configuration settings for the MDX document, such as titles. **Slug** refers to a short, URL-friendly, and human-readable string that represents a piece of content.</Message>
